...
public class DynamicChatClientFactory {
    ...
    public DynamicChatClientFactory(CourseTool courseTool,
    ...
    /**
     * 根据用户提供的 API 配置动态创建普通 ChatClient
     */
    public ChatClient createChatClient(String apiKey, String baseUrl, String model) {
        ...
        log.info("动态创建 ChatClient: baseUrl={}, model={}", baseUrl, model);
    ...
    /**
     * 根据用户提供的 API 配置动态创建带工具调用能力的 Agent ChatClient
     */
    public ChatClient createAgentChatClient(String apiKey, String baseUrl, String model) {
        OpenAiApi api = new OpenAiApi(baseUrl, apiKey);
        OpenAiChatOptions options = OpenAiChatOptions.builder()
                .model(model != null && !model.isBlank() ? model : "qwen-turbo")
                .temperature(0.7)
        ...
        log.info("动态创建 Agent ChatClient: baseUrl={}, model={}", baseUrl, model);
        return ChatClient.builder(chatModel)
                .defaultSystem(AGENT_SYSTEM_PROMPT)
                .defaultFunctions(buildFunctionCallbacks())
                .build();
    }

    private FunctionCallback[] buildFunctionCallbacks() {
